...
Umami is the only website analytics tool. Every full content page loads `https://wave.21nauts.com/script.js` with website ID `59e34c79-c9e1-4936-b4e7-48d98fd0b556`. Configuration lives in `site.analytics`; the shared HTML head includes the deferred script in both preview and release builds. `data-domains` restricts collection to `knistertap.com` and `www.knistertap.com`, excluding local previews and the GitHub hostname. Redirect-only pages do not emit a second pageview. Fonts and artwork are self-hosted, with no advertising trackers, newsletter form, or external embedded media. Native details elements provide mobile navigation, FAQ disclosure, and the research note. The privacy draft records the active Umami connection; instance operator, infrastructure location and retention details still need confirmation.
